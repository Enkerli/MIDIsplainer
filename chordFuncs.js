...
// Update the chord display function to include working clickable links
function updateChordDisplay() {
    ...
    if (inversions.length > 0) {
        const altSpellings = inversions
            ...
            .map(spelling => {
                ...
                return `<a href="#" data-chord="${chordParam}" class="chord-link">${spelling.chord}</a> [${spelling.notes.join(', ')}]`;
            });
        ...
        const trueInversions = inversions
            ...
            .map(inv => {
                ...
                return `<a href="#" data-chord="${chordParam}" class="chord-link">${inv.chord}</a> [${inv.notes.join(', ')}]`;
            });
    ...
    document.querySelectorAll('.chord-link').forEach(link => {
        link.onclick = function(e) {
            e.preventDefault();
            const chordParam = this.getAttribute('data-chord');
            if (chordParam) {
                window.history.pushState({}, '', `?chord=${chordParam}`);
                handleChordParam(decodeURIComponent(chordParam));
            }
        };
    });
}
